...
// -------------------- Disabled Uno Slave Mode --------------------
// When platform_sample_on_board = false, performAutoSample() self-acknowledges with a
// simulated :X1 after replace_water_sample_delay seconds instead of waiting for the real
...
void performAutoSample() {
    ...
    while ((millis() - waitStart < TIMEOUT_MS) && !stopMode) {
        ...
        if (!platform_sample_on_board &&
            (millis() - waitStart) >= (replace_water_sample_delay * 1000UL)) {
            Serial.println("platform_sample_on_board=false, self-acking :X1 after "
                           + String(replace_water_sample_delay) + "s");
